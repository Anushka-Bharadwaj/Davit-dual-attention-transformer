...
# --- Dual Attention Block for DaViT ---
class DualAttentionBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4.0, drop=0.0):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.spatial_attn = nn.MultiheadAttention(dim, num_heads, batch_first=True, dropout=drop)
    ...
    def forward(self, x):
        ...
        # Channel Attention
        x_norm = self.norm2(x)
        channel_attn = self.channel_attn(x_norm)
        x = x + x_norm * channel_attn
...
class DaViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, emb_dim=768, depth=12, heads=12, mlp_ratio=4.0, num_classes=1000, drop=0.0):
        super().__init__()
        img_size = (img_size, img_size) if isinstance(img_size, int) else img_size
        patch_size = (patch_size, patch_size) if isinstance(patch_size, int) else patch_size
        self.patch_embed = PatchEmbedding(emb_dim, img_size, patch_size, 3)
        self.pos_embed = PositionalEmbedding(self.patch_embed.n_patches + 1, emb_dim)
        self.cls_token = nn.Parameter(torch.randn(1, 1, emb_dim))
        self.blocks = nn.Sequential(*[
            DualAttentionBlock(emb_dim, heads, mlp_ratio, drop) for _ in range(depth)
        ])
...
lr = 0.005
epochs = 5
save_dir=r"C:\Users\Hp\CascadeProjects\vision_transformer\DaViT"
